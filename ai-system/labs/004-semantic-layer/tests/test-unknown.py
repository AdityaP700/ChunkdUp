...
def test_unknown_memory():
    ...
    # 3. Query
    query = "What's my favourite database?"
    print(f"Query: {query}")
    ...
    # 5. Output
    print("\n=== EXPECTED ===")
    print('{"answer": "I don\'t know.", "citations": []}')
    ...
    if "don't know" in answer or "do not know" in answer:
        print("\n[PASSED]: LLM successfully prevented hallucination.")
    else:
        print("\n[FAILED]: LLM hallucinated an answer.")
        
    if len(citations) == 0:
        print("[PASSED]: Citations are empty as expected.")
    else:
        print("[FAILED]: Citations were provided for an unknown memory.")
        
    # Store the results in the data dir
    results_dir = os.path.join(labs_dir, "data", "tests-lab4")
    os.makedirs(results_dir, exist_ok=True)
    result_path = os.path.join(results_dir, "no_memory.json")
